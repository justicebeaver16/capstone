...
const Dashboard = () => {
  ...
  const reduxGuests = useSelector(selectAllGuests);
      ...
      try {
        ...
        const enrichedEvents = await Promise.all(
          (data.Events || []).map(async (event) => {
            const vendorsRes = await fetch(`/api/vendors/${event.id}`);
        ...
        {events.length > 0 ? events.map((event) => (
          <div className="event-card" key={event.id}>
            ...
            <section className="dashboard-section">
              ...
              <button className="dashboard-link-button" onClick={() => navigate('/tasks')}>
                Add Tasks
              </button>
            </section>
            ...
            <section className="dashboard-section">
              ...
              <button className="dashboard-link-button" onClick={() => navigate('/vendors')}>
                View Vendors
              </button>
            </section>
          </div>
